...
washdc_map <- get_stamenmap(bbox = c(left = -77.1323, bottom = 38.7889, right = -76.9319, top = 39.1236),
                            maptype = "terrain",
                            zoom = 11)
...
ggmap(washdc_map) +
  geom_point(data = Trips1,
             ...
             alpha = .1) +
  theme_map()
# Ask about this question why there aren't points around lat = 39.1
```
...
```{r}
census_pop_est_2021 <- read_csv("https://www.dropbox.com/s/6txwv3b4ng7pepe/us_census_2021_state_pop_est.csv?dl=1") %>% 
  separate(state, into = c("dot","state"), extra = "merge") %>% 
  select(-dot) %>% 
  mutate(state = str_to_lower(state))

covid_most_recent %>%
  left_join(census_pop_est_2021,
            by = "state") %>%
  mutate(cases_per_10000 = (cases/est_pop_2021)*10000) %>%
  ggplot() +
  geom_map(map = states_map,
  ...
  labs(title = "Most recent COVID-19 case counts in US",
       ...
       fill = "") +
  theme_map()
# Ask how to get the population of 2021; changing 2018 to 2021 doesn't do anything.
```
...
# Ask about the title of the legend
```
